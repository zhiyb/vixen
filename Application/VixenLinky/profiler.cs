...
namespace VixenLinky
{
	public class Profiler
	{
		...
		private long _controllerCount = 0;
		public long ControllerCount {
			get { return _controllerCount; }
			set { _controllerCount = value; }
		}
		...
		{
			_sw.Restart();
			long sleep = _sw.ElapsedMilliseconds, ms_prev = sleep;
			long fcnt = FileCount, ccnt = ControllerCount;
			while (!_stop) {
				...
				long nfcnt = FileCount, nccnt = ControllerCount;

				long ms = _sw.ElapsedMilliseconds;
				long diff = ms - ms_prev;
				ms_prev = ms;

				double ffps = (double)((nfcnt - fcnt) * 1000) / diff;
				double cfps = (double)((nccnt - ccnt) * 1000) / diff;
				fcnt = nfcnt;
				ccnt = nccnt;

				rec.values = new List<Tuple<string, double> >();
				rec.values.Add(new Tuple<string, double>("FileIO FPS", ffps));
				rec.values.Add(new Tuple<string, double>("Controller FPS", cfps));
				rec.time = ms;
				_records.Add(rec);
				sleep += _itvl;
				if (sleep - ms > 0)
