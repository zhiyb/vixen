...
{
    public class RawWriter : IExportWriter
    {
        ...
        public void WriteNextPeriodData(List<Byte> periodData)
        {
            if (_dataOut != null)
            {
                try {
                    ...
                    _dataOut.Write(new byte[] { 0xde, 0xad, 0xbe, 0xef, 0x02, 0x00 });
                    _dataOut.Write((UInt16)_seqNumChannels);
                    _dataOut.Write(periodData.ToArray(), 0, _seqNumChannels);
                    /*for (int i = 0; i != _seqNumChannels; i++)
                    {
                        _dataOut.Write((UInt16)i);
                        _dataOut.Write(periodData[i]);
                    }*/
